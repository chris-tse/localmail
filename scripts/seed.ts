...
const seed = Effect.gen(function* () {
  ...
  const mockMessages: MockMessage[] = [
    ...
    {
      subject: "Re: Q3 Project Timeline Review",
      bodyText: "Good point on the prototype date. Aug 8 works. I'll coordinate with Priya on user testing — she has the research panel ready.\n\nUpdated timeline attached (not really, this is mock data).\n\nSarah",
      threadGroup: "q3-timeline",
    },
    {
    ...
    },
    {
      ...
      bodyHtml: "<h2>Design System Update</h2><p>The updated design system is live in Figma.</p><ul><li>Data table component with sorting/filtering</li><li>Updated color palette</li><li>New icon set (200+ icons)</li><li>Dark mode variants</li></ul>",
    },
    {
    ...
    },
    {
      ...
      bodyText: "Nice catch Carlos. I'll review the PR today. We should also add a test case for this — I don't think our rate limit tests cover preflight requests at all.\n\n- Alex",
      threadGroup: "bug-rate-limit",
    },
    {
      subject: "Lunch next Tuesday?",
      bodyText: "Hey! Long time no chat. Want to grab lunch next Tuesday? That new ramen place on 5th opened and I heard it's amazing.\n\n- James",
    },
    {
      ...
      bodyHtml: "<div style='font-family: sans-serif'><h2>Your subscription is expiring</h2><p>Expires <strong>July 30</strong>.</p><p>Renewal: <strong>$199/year</strong> (code <code>RENEW20</code>)</p></div>",
    },
    {
    ...
    },
    {
      ...
      bodyText: "On it. The auth token in logs issue is from the debug middleware — I'll strip sensitive fields from the request logger.\n\nShould have a PR up by EOD.\n\n- Alex",
      threadGroup: "security-audit",
    },
  ]
  ...
  for (let i = 0; i < mockMessages.length; i++) {
    ...
    if (msg.subject.includes("Invoice") || msg.subject.includes("analytics report")) {
      const attId = ulid()
      const filename = msg.subject.includes("Invoice") ? "invoice-1087.pdf" : "june-campaign-report.xlsx"
      const contentType = msg.subject.includes("Invoice") ? "application/pdf" : "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
...
Effect.runPromise(
  seed.pipe(Effect.provide(SqliteLive)),
).catch((err) => {
